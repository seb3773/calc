...
# ==============================================================================
# Script de mise à jour du dépôt APT GitHub Pages pour calc (tde-calc)
# ==============================================================================

REPO_DIR="$(cd "$(dirname "$0")" && pwd)"
PAGES_DIR="/tmp/calc_gh_pages_$$"
REMOTE_URL=$(git config --get remote.origin.url)

echo "=================================================="
echo " calc - APT Repository Builder (gh-pages)"
...
# Find deb packages at repo root or build dir
shopt -s nullglob
...
if [ ${#DEB_FILES[@]} -eq 0 ]; then
    echo "Error: No .deb package found at repo root. Please run ./build_deb.sh first."
    exit 1
fi
...
POOL_DIR="$PAGES_DIR/pool/main/t/tde-calc"
...
shopt -s nullglob
QSI_FILES=($(ls -t "$REPO_DIR"/setup_tde-calc_*.qsi "$REPO_DIR"/setup_*.qsi 2>/dev/null || true))
if [ ${#QSI_FILES[@]} -eq 0 ]; then
    QSI_FILES=($(ls -t "$REPO_DIR"/*.qsi 2>/dev/null || true))
fi
shopt -u nullglob
...
shopt -s nullglob
APPIMAGE_FILES=($(ls -t "$REPO_DIR"/tdecalc-*.AppImage "$REPO_DIR"/tdecalc_*.AppImage 2>/dev/null || true))
if [ ${#APPIMAGE_FILES[@]} -eq 0 ]; then
    APPIMAGE_FILES=($(ls -t "$REPO_DIR"/*.AppImage 2>/dev/null || true))
fi
shopt -u nullglob
...
apt-ftparchive \
  -o APT::FTPArchive::Release::Origin="calc" \
  -o APT::FTPArchive::Release::Label="calc APT Repository" \
...
echo " URL: https://seb3773.github.io/calc/"
echo "=================================================="
