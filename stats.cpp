...
KNumber KStats::median(void) {
	...
	if( bound & 1) {  // odd
	  ...
	  index = bound / 2;
	  if (index >= tmp_mData.size() || index == 0) {
			error_flag = true;
			return 0;
	  }
...
KNumber KStats::std_kernel(const KNumber & _mean)
{
...
KNumber KStats::std(void)
{
	...
	KNumber m = mean();
	return (std_kernel(m) / KNumber(count())).sqrt();
}
...
KNumber KStats::sample_std(void) {
	...
	KNumber m = mean();
	result = (std_kernel(m) / KNumber(count() - 1)).sqrt();
...
bool KStats::hasError() const {
  return error_flag;
}

void KStats::clearError() {
  error_flag = false;
}
