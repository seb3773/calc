...
const KNumber & CalcEngine::deg_to_rad_factor() {
    static const KNumber factor = KNumber::Pi / KNumber(180);
    return factor;
}
const KNumber & CalcEngine::gra_to_rad_factor() {
    static const KNumber factor = KNumber::Pi / KNumber(200);
    return factor;
}
const KNumber & CalcEngine::rad_to_deg_factor() {
    static const KNumber factor = KNumber(180) / KNumber::Pi;
    return factor;
}
const KNumber & CalcEngine::rad_to_gra_factor() {
    static const KNumber factor = KNumber(200) / KNumber::Pi;
    return factor;
}
...
void CalcEngine::ArcTangensDeg(KNumber input)
{
	if (input.type() == KNumber::SpecialType) {
		...
		else if (input == KNumber("inf")) _last_number = KNumber(90);
		else if (input == KNumber("-inf"))  _last_number = KNumber(-90);
		return;
	}
...
void CalcEngine::ArcTangensGrad(KNumber input)
{
	if (input.type() == KNumber::SpecialType) {
		if (input == KNumber("nan")) _last_number = KNumber("nan");
		else if (input == KNumber("inf")) _last_number = KNumber(100);
		else if (input == KNumber("-inf"))  _last_number = KNumber(-100);
		return;
	}
...
void CalcEngine::AreaCosHyp(KNumber input)
{
	if (input.type() == KNumber::SpecialType) {
		if (input == KNumber("nan")) _last_number = KNumber("nan");
		else if (input == KNumber("inf")) _last_number = KNumber("inf");
		else if (input == KNumber("-inf"))  _last_number = KNumber("nan");
		return;
	}
...
void CalcEngine::Exp(KNumber input)
{
	if (input.type() == KNumber::SpecialType) {
		...
		else if (input == KNumber("-inf"))  _last_number = KNumber::Zero;
		return;
	}
	CALCAMNT tmp_num = static_cast<double>(input);
...
{
	if (input.type() == KNumber::IntegerType) {
		unsigned long int val = static_cast<unsigned long int>(input);
		if (val > 100000) {
			return KNumber("inf");
		}
		mpz_t gmp_result;
		mpz_init(gmp_result);
		mpz_fac_ui(gmp_result, val);
		
		char *tmp_ptr;
		gmp_asprintf(&tmp_ptr, "%Zd", gmp_result);
		KNumber result(tmp_ptr);
...
void CalcEngine::TangensHyp(KNumber input)
{
	if (input.type() == KNumber::SpecialType) {
		if (input == KNumber("nan")) _last_number = KNumber("nan");
		else if (input == KNumber("inf")) _last_number = KNumber::One;
		else if (input == KNumber("-inf"))  _last_number = KNumber::MinusOne;
		return;
	}
...
bool CalcEngine::evalStack(void)
{
	...
	if (_stack.isEmpty()) {
		TQMessageBox::critical(0L, tr_str("Error"), tr_str("Stack processing error - empty stack"));
		return false;
	}
...
TQString CalcEngine::getExpression() const
{
	TQString expr;
	for (TQValueList<CalcEngine::_node>::ConstIterator it = _stack.begin(); it != _stack.end(); ++it)
	{
		if ((*it).operation == FUNC_BRACKET) {
			expr += " (";
			continue;
		}
		
		TQString num_str = (*it).number.toTQString(9);
		
		TQString op_str;
		switch((*it).operation) {
			case FUNC_ADD: op_str = " + "; break;
			case FUNC_SUBTRACT: op_str = " - "; break;
			case FUNC_MULTIPLY: op_str = " * "; break;
			case FUNC_DIVIDE: op_str = " / "; break;
			case FUNC_MOD: op_str = " Mod "; break;
			case FUNC_POWER: op_str = " ^ "; break;
			case FUNC_AND: op_str = " AND "; break;
			case FUNC_OR: op_str = " OR "; break;
			case FUNC_XOR: op_str = " XOR "; break;
			case FUNC_LSH: op_str = " Lsh "; break;
			case FUNC_RSH: op_str = " Rsh "; break;
			default: break;
		}
		expr += num_str + op_str;
	}
