...
class _knumerror : public _knumber
{
 public:
  ...
  {
    auto const *ptr = dynamic_cast<_knumerror const *>(&num);
    if (!ptr) {
#ifdef __EXCEPTIONS
      throw std::bad_cast();
#else
...
class _knuminteger : public _knumber
{
 public:
  ...
  virtual void copy(_knumber const & num)
  {
    auto const *ptr = dynamic_cast<_knuminteger const *>(&num);
    if (!ptr) {
#ifdef __EXCEPTIONS
      throw std::bad_cast();
#else
  ...
  _knuminteger * intXor(_knuminteger const &arg2) const;
...
class _knumfraction : public _knumber
{
 public:
  ...
  virtual void copy(_knumber const & num)
  {
    auto const *ptr = dynamic_cast<_knumfraction const *>(&num);
    if (!ptr) {
#ifdef __EXCEPTIONS
      throw std::bad_cast();
#else
...
class _knumfloat : public _knumber
{
 public:
  ...
  virtual void copy(_knumber const & num)
  {
    auto const *ptr = dynamic_cast<_knumfloat const *>(&num);
    if (!ptr) {
#ifdef __EXCEPTIONS
      throw std::bad_cast();
#else
