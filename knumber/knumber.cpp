...
{
  std::unique_ptr<_knumber> temp;
  switch(num.type()) {
  case SpecialType:
    temp.reset(new _knumerror(*(num._num)));
    break;
  case IntegerType:
    temp.reset(new _knuminteger(*(num._num)));
    break;
  case FractionType:
    temp.reset(new _knumfraction(*(num._num)));
    break;
  case FloatType:
    temp.reset(new _knumfloat(*(num._num)));
    break;
  }
  _num = temp.release();
}
...
KNumber const & KNumber::operator=(KNumber const & num)
{
  ...
  switch(num.type()) {
  case SpecialType:
    temp.reset(new _knumerror());
    break;
  case IntegerType:
    temp.reset(new _knuminteger());
    break;
  case FractionType:
    temp.reset(new _knumfraction());
    break;
  case FloatType:
    temp.reset(new _knumfloat());
    break;
  }
...
KNumber const KNumber::operator^(KNumber const & arg2) const
{
  ...
  tmp_num._num = tmp_arg1->intXor(*tmp_arg2);
